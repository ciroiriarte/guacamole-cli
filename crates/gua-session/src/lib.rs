...
impl Session {
    ...
    fn ack_stream_blob(&mut self, stream: String) -> Result<()> {
        ...
        // guacd bounds the unacknowledged backlog; when it fills, tee mode drops
        // output while raw mode throttles the session and, if acks stop entirely,
        // eventually disconnects. Acks are therefore
